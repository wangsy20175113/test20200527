...
var app = new Vue({
	...
	methods:{
		toggleCity(p,index)//cityDisplayed第一次是被在此处定义（它是靠toggleCity这个点击的方法定义的）,when we click like 内蒙古，then we have a undefined variable which is cityDisplayed
		{
			if(!p.cityDisplayed)//此处cityDisplayed 是undefined,所以not undefined 就是true,把cityDisplayed赋值为true,
// 			//下一次再点击内蒙古时，cityDisplayed是true,所以此处！cityDisplayed为false，走到else分支，cityDisplayed被赋值为false.
			{
